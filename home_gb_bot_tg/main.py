...
def random_choise(list_X,i = 0):
    if i == 0:
        return choice(list_X)
    else: return list_X[i]
...
def mess(message):
    if message.text == 'Pic':
            ...
            bot.send_audio(message.chat.id,audio=open(f'D:\\GBPython\\audio\\{random_choise(audio_set)}','rb'))
